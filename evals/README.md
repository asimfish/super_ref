...
## run_citation_unit_evals.py（240 项）

单元/属性层：解析（BibTeX、注册库、落地页元数据）、归一化（作者定向、变音符、venue 别名）、策略校验（角色/来源家族地板）、传输安全（URL/DNS/重定向/字节上限、gzip 解码与炸弹拒收、注册库限流的同主机节流与有界 Retry-After 重试）、路径遏制。回归定位到函数级。
